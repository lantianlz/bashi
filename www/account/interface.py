...
class UserBase(object):
    ...
    def send_confirm_email(self, user):
        ...
        if not cache_obj.get_time_is_locked(key, 60):
            ...
            async_send_email(user.email, u'巴适二手车邮箱验证', utils.render_email_template('email/verify_email.html', context), 'html')
    ...
    def send_forget_password_email(self, email):
        ...
        if not cache_obj.get_time_is_locked(key, 60):
            ...
            async_send_email(email, u'巴适二手车找回密码', utils.render_email_template('email/reset_password.html', context), 'html')
        return 0, dict_err.get(0)
    ...
    def get_user_by_external_info(self, source, access_token, external_user_id,
        ...
        if et:
            ...
            email = '%s_%s@mr84che.com' % (source, int(time.time() * 1000))
    ...
    def get_count_group_by_create_time(self, count=360):
        ...
        sql = """
            ...
            FROM account_bashi.account_user 
